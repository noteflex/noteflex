...
| 2026-06-22 | 87 | 87 | 174 | 아티큘레이션(articulation) — 스타카토(·)·스타카티시모(▾)·테누토(—)·악센트(>)·마르카토(^)·레가토(슬러) 여섯 기본 기호의 정의·시각 표기·결합 규칙(포르타토 = 점+테누토, 악센트+테누토, 마르카토+슬러 등) + 오케스트라 다중 파트 동시 지시 관습 + 슬러 그룹 첫 음 적용·기둥 반대편 표기 관습 (ko+en) 추가. 화이트리스트(기호 정의·규칙) 부합, 학술 인용 없음. 이미지 출처 Wikimedia Commons 3종(Nick Daly 다섯 기호 도식 + Beethoven Sym.6 자필 발췌 + Tjako van Schie 아티큘레이션 스펙트럼). BlogPostCTA 마커("## 이미지 출처"/"## Image Sources") 정확 일치, prerendered ko·en 2편 생성·#root 본문·핵심 키워드(아티큘레이션 18·스타카토 20·레가토 18·테누토 19·마르카토 17·악센트 20 / articulation 16·staccato 15·legato 11·tenuto 11·marcato 10·accent 18) 확인. |
...
| 2026-06-22 | 아티큘레이션 — 스타카토·스타카티시모·테누토·악센트·마르카토·레가토 여섯 기본 기호 + 결합 규칙(점+테누토 = 포르타토, 악센트+테누토, 마르카토+슬러, 악센트+슬러) + 오케스트라 다중 파트 관습 + 슬러 그룹 첫 음만 적용 / 기둥 반대편 표기 (ko+en, slug `articulation-markings-explained`). 이미지: Wikimedia Commons 3종 — Nick Daly 다섯 기호 도식(staccato·staccatissimo·marcato·accent·tenuto) + Beethoven Sym.6 1807 자필 발췌(staccato/legato 대비) + Tjako van Schie 아티큘레이션 스펙트럼(legato → staccatissimo). curl HTTP 200 PASS. prerender ko·en 2편 생성·#root 본문·키워드 grep 통과. 학술 인용 없음(검수 0 정책). 화이트리스트(기호 정의·규칙) 부합. | (펜딩) |
| 2026-06-25 | 잇단음표 — 셋잇단음표(triplet, 3:2)·오잇단음표(5:4)·여섯잇단음표(sextuplet)·일곱잇단음표(7:4)·아홉잇단음표(9:8 또는 9:4)의 정의·박자 안 분할 방식. 8분 셋잇단음표가 기준(한 박을 1/3씩 세 등분), 4분·2분·16분 셋잇단음표는 같은 원리로 박 단위 확장. 표기 3종(숫자만 / 숫자+괄호 / 숫자+슬러), 숫자 표기 양식 2종(단순 숫자 vs 비율 표기 3:2·5:4·7:4), 쉼표가 그룹 슬롯에 들어가는 케이스, 빔 반대편 위치 관습 (ko+en, slug `tuplets-explained`). 이미지: Wikimedia Commons 2종 — jobu0101 Achteltriole 8분 셋잇단음표 도식 + Achtelkette als Triole 8분 두 묶음 vs 셋잇단음표 비교 도식. curl HTTP 200 PASS. prerender ko·en 2편 생성·#root 본문·키워드 grep 통과(잇단음표 31·셋잇단음표 24 / tuplet 20·Triplet 11). 학술 인용 없음(검수 0 정책). 화이트리스트(리듬 표기·규칙) 부합. ko 2498자 / en 1199 단어. | (펜딩) |
