...
> **출시 마감**: 🎯 ~~**2026-05-31**~~ → **2026-06-01** (사업자 등록 일정 영향)
...
- 세션·일일 제한·24h 카운트다운은 건드리지 말 것(이번 변경 대상 아님).
- 관련 UI 정합: LevelSelect 잠금 카드, Pricing, 게이팅 문구.
...
- [위생] 세션 끝 git status로 untracked 확인(src/daily 통째 미커밋 재발 방지).
...
## 2026-06-06 인증 흐름 개선 후속

- [출시후-관찰] **인앱 브라우저 한계 — 원래 탭 수동 새로고침 필요** — 메일 앱이 BroadcastChannel 미지원 브라우저(일부 iOS 인앱 브라우저)로 링크를 열면 인증은 성공하지만 원래 탭은 자동 새로고침 안 됨. 콜백 탭 자체는 로그인된 홈으로 정상 도착. 알려진 한계. 사용자 민원 발생 시 "페이지를 새로고침하세요" 안내 문구 추가 고려.
- [출시후-관찰] **기존 유저 다른 탭 인증 완료 시 원래 탭 홈 강제 이동** — `App.tsx` `AuthBroadcastListener`가 BC 신호 수신 시 항상 `/` 또는 `/welcome`으로 navigate. 기존 로그인 유저가 다른 탭에서 재로그인하면 현재 탭이 홈으로 이동되는 부작용. 운영 관찰 후 거슬리면 "모달만 닫기(페이지 이동 없음)"로 수정.
- [수동 작업 필요] **Magic Link 이메일 템플릿에 `{{ .Token }}` 블록 반영 확인** — 6자리 코드 폴백은 Supabase가 이메일에 OTP 토큰을 포함해야 동작. Supabase 대시보드 > Authentication > Email Templates > "Magic Link" 템플릿에 `{{ .Token }}` 블록이 있는지 확인 (기본 템플릿 미포함 시 추가 필요).
- [출시후-관찰] **만료 링크 에러 화면 운영 확인 잔여** — `AuthCallback`에 expired/generic 에러 화면 신규 추가(2026-06-06). 실제 만료 링크 클릭 시 "링크가 만료됐어요" 화면 + 재발송 CTA가 올바르게 노출되는지 라이브 환경 검증 필요.

## 2026-06-06 Free→Pro 전환 후속

- [출시후] **Free→Pro 전환 직후 프레이밍** — 건너뛴 sub2·sub3 백필을 안내하는 카피/화면 1개 필요 ('입증된 실력은 Fast Track으로 빠르게 통과' 톤). 결제 직후 '뒤로 보내진' 인상 방지. 첫 실전 전환자 발생 시점에 착수.
- [출시후] **전환 후 클리어 스테이지 표시 모순** — Free 시절 클리어한 2-1·3-1이 Pro 진행도 게이트에 의해 '잠김'으로 표시됨(레코드는 보존, 재플레이는 허용). 클리어 이력이 있는 칸은 잠김 대신 '클리어됨' 계열 표시로 정리 검토. 1번과 동일 시점에.
- [출시후-관찰] **Fast Track 발동 조건 하향 조정 고려** — 현행 오류 ≤1% AND 시간 ≤50%는 보수적(30노트 기준 사실상 무실수 요구). 전환자 백필 구간에서 정작 필요할 때 발동 안 할 위험. 첫 전환자들의 백필 통과 데이터 확보 후 임계값 재조정. scoring-rules.md '조정 후보' 항목과 연동.
...
- [출시후-모니터링] **AdSense 사이트 심사 대기 중** (5/24 제출, 정상 범위 2~4주)
  - publisher ID 스니펫: `index.html` 정적 삽입, `VITE_ADS_ENABLED` 무관, 라이브 확인 완료
  - ads.txt: `pub-4314740126698954`, 라이브 확인 완료 (Google 재크롤 대기)
  - 기술 조치 불요. 대시보드 "주의 필요" 플래그만 모니터링 — 뜨면 그때 대응.
...
- [✅ 2026-06-16] **개인 분석 read 전체 admin RLS user_id 필터 audit (closed)** — 클라이언트 read 20개 경로 전수 점검: `usePeriodReport`·`useDailyIntervals`·`useWeeklyReport`(daily rows)·`useMonthlyReport`(day/week rows)·`useMyStats`(sessions/daily/notes)·`useUserStats`·`useStreak`(streaks/activity)·`useUserMastery`·`useMasteryDetails`·`useUserWeakScores`·`useLevelProgress`·`NextStepCard`(user_note_status)·`fetchUserNoteLogs` 모두 이미 `.eq("user_id", userId)` 명시 필터 보유. `get_daily/weekly/monthly_report` RPC 3종은 own-only 가드. 잔존 0. 코드 변경 없음. `/admin/*` 전용 `useAdminUserDetail`은 외부 userId 의도이므로 미수정.
...
- [출시후-중기 sprint] **vitest 풀 스위트 OOM 해소 — 진짜 원인은 컴포넌트 cleanup 누수** — 2026-06-01 config 3단계 진단 결과(commit X — 효과 없어 미적용): (1) default 병렬 worker 4 GB → OOM, (2) singleFork + 8 GB heap → 8 GB까지 차서 OOM, (3) maxForks=2 × 8 GB heap → 1126s 후 한 worker 8 GB 도달, (4) maxForks=1 × 12 GB heap → 그래도 OOM. config·heap 튜닝으로 해소 불가능 확인. **진짜 원인**: NoteGame 테스트가 mount하는 컴포넌트 트리에서 unmount 시 정리 안 되는 reference 누수 (`useNoteLogger` Supabase 구독, `useUserMastery`/`useUserWeakScores` React Query 캐시, `tone.js` AudioContext, `setInterval/setTimeout` cleanup 미흡 등). 진짜 fix는: (a) `vi.mock('@/lib/sound')` `vi.mock('@/integrations/supabase/client')` 도입, (b) `afterEach(cleanup)` from `@testing-library/react`, (c) `vi.clearAllTimers()` + `vi.useRealTimers()` 정리, (d) NoteGame.test.tsx 무거운 케이스를 단위 hook 테스트로 분해. 분량 추정: 4~8 시간. 영향: 풀 회귀 검증 자산 복구·CI 도입 가능. 단기 우회는 그대로 — 좁힌 vitest (`npx vitest run src/lib src/hooks src/components/practice src/components/coaching`)로 OK.
...
- [출시후] **LangToggle 헤더 마운트 재검토** — ja·zh 언어 추가 시. 현재 ko/en만 운영, IP 자동 감지(timezone Asia/Seoul + navigator.language ko-* → ko, else en)로 guest UX 충분. UI 미니멀 톤 + 한국 색채 회피 의도로 마운트 안 함. ja·zh 추가 후에는 자동 감지만으로 부족 → 헤더 또는 footer 우측 LangToggle 노출 검토 필요. LangToggle 컴포넌트(src/components/LangToggle.tsx)는 작성된 상태 유지.
- [출시후] **모바일 햄버거 메뉴 (Navigation Sub-step 4)** — Vercel Analytics 사용자 행동 데이터 확인 후 진짜 필요 시 추가. 현재 footer가 충분한 nav 역할, ROI 낮음. (Navigation 정비 Sub-step 1·2·3 완료: 9e57317 Skip to content, 56c74f6 Footer 색·위계, 666ccba 데스크탑 헤더 nav)
- [출시후 3~6개월차] **자체 visit_logs 시스템** — 출시 직후~3개월은 Vercel Analytics(PV·DAU·디바이스·referrer 기본 지표)로 충분. 출시 후 3~6개월차에 사용자 행동·게임 join(user_id) 퍼널·코호트 분석 같은 정교 분석이 필요해지면 자체 visit_logs 시스템 구축. 5/31 사전 설계 1단계 완료: 테이블(visit_logs id·session_id·user_id·page_path·referrer·device_type·visited_at) + 인덱스 + RLS(anon insert/admin select) + 90일 pg_cron cleanup + useVisitLogger hook + /admin/visits 차트(DAU·PV·재방문·페이지 top10·디바이스·referrer) + 아바타 메뉴 분석 완료.
...
- [ ] **게임 영역 검증**
  ...
  - Anchor 다이얼로그 작성
  - [x] 실제 게임 플레이 후 기록 적재 확인 ✅ (2026-05-21 완료 — RPC·트리거·reaction_ms 누적·calibration 4개 영역 fix)
- [ ] **PWA 구현**
  - manifest.json 작성
  - vite-plugin-pwa 설정
  - service worker
...
### 2026-05-21 신규 추가 항목 (출시 전 필수)

- [ ] **batchSize 정책 변경 + UI 사이즈 통일** (출시 차단 영역)
  - Lv1~4: sub1(2,3) / sub2(3,5) / sub3(3,5,7)
  - Lv5~7: 모두 3,5,7
...
### 2026-05-21 신규 추가 항목 (출시 후)

- [ ] **CalibrationModal.tsx 파일 자체 제거** (현재는 롤백 대비 유지)
- [ ] **localStorage V1 키 cleanup 코드 제거** (마이그 완료 후)
- [ ] **블로그 본문 콘텐츠 깊이 보강** (학술 인용·키워드 강화·내부 링크)
- [ ] **SSR/SSG 전환 검토** (Next.js 마이그) — 트래픽 보고 결정
...
## 2026-05-16 통합 fix 추가 PENDING

### 출시 후 즉시 (6월~)

- [ ] **noteComparison 윈도우 튜닝** — currentWindow=30·previousWindow=120 기본값
...
### B.4 일일 세션 한도 시스템 ✅ Group B 완료 + Fix Sprint (2026-05-09)
...
**Fix Sprint 후속 (2026-05-09 오후~)**:
- 사용자 검증 발견: 백그라운드 게임 진행 (audio·timer 초기화) + 보조 버튼 escape hatch + 메시지 단순
- 게이트 위치 이동: NoteGame 마운트 → **LevelSelect 단계 클릭 시점** (commit b58d873)
  - LevelSelect handleSelect 내 hasReached 체크 + DailyLimitModal 노출
...
- D1: `user_sublevel_progress.fast_track BOOLEAN` 컬럼 + `record_sublevel_attempt` RPC 패스트트랙 분기 (premium/admin + sublevel≥2 + 첫 세션 + 정답률≥99% + avg_reaction≤0.5 → passed=true 강제) + `get_mastery_score` fast_track=true → 100 강제 + tier 조회 정정
- D2: `SublevelProgress.fast_track` 타입 + `useLevelProgress` 추출 + `computeMasteryScore` 100 강제 + `Index.tsx` fastTrack 전달
- D3: `SublevelPassedDialog` fastTrack 분기 + 5초 자동 진입 카운트다운 (autoAdvancedRef 1회 보장) + `aiCoaching.ts` fastTrack 우선 분기

**마이그레이션 적용**: Docker 오프라인 → `supabase/migrations/20260509_fast_track.sql` 작성 완료. 사용자가 Supabase 대시보드 또는 CLI(`supabase db push --db-url ...`)로 production apply 필요.
...
## §X.UI 전체 UI 정비 sprint 🟡 (출시 전 또는 출시 후, 별도 AI 협업)

**사용자 결정 (2026-05-09)**: 전체 UI 허접 영역 인지. 기능 위주 개발 우선 → UI 정비는 별도 sprint.

**정비 대상**: MasteryScoreCard · MasteryHeroCard · SublevelPassedDialog · GameOverDialog · DailyLimitModal · LevelSelect · Pricing · Dashboard · Landing 등 전 영역

**정비 범위**: 색상·간격·폰트·아이콘·차트 디자인·진행률 바·여백·전체 톤앤매너

**디자인 톤**: 메모리 #25 스티브잡스 스타일 일관 (compact·세련·명확·즉시 시각 반응)

**도구**: UI 개발 AI 별도 협업 (사용자 결정)

**우선순위**: 출시 전 또는 출시 후 별도 sprint — 시점은 사용자 결정
...
### 3.1 ⏳ AdSense 심사 (2026-05-14~16 신청 예정)
- **시점**: 2026-05-14~16 (누적 30편+ 도달 + 사업자 등록 정정 완료 후)
  - 사업자 등록 정정 진행 중 (사업자명 + 업종코드 추가)
- **심사 중 조건**: 광고 슬롯 실제 노출 필수 (`VITE_ADS_ENABLED=true` + 실제 Publisher ID)
  - 머니 스위치 6·7·8번 = 출시 시점 X → **심사 신청 시점에 적용**
- 심사 1~2주 → 5/28~6/2 통과 예상 (출시 5/31 전후)
- **사용자가 블로그 글 틈틈이 작성** (2026-05-04 기준 24편, 목표 30편)
...
- [ ] §5.2 회원관리 페이지
- [ ] §5.4 가입자 대시보드 잠금
- [ ] §10.1.1 약관 임시 텍스트 → Termly/iubenda 생성 → 번역·검수
- [ ] §10.1 약관 완성 → ⏳ Paddle Vendor 심사 신청
- [ ] **사용자 작업**: 블로그 글 누적 30개 → ⏳ AdSense 심사 신청 (5/14~16)
