...
> **2026-06-13 정책 재설계 — 검수 0 전제**
> 사용자는 글을 검수하지 않고 그대로 push. 정책 자체가 검수 없이 안전하도록 재설계되어야 함.
> 핵심 변화 4가지: ① 학술 인용 전면 금지 ② 토픽 화이트리스트(위키 수준 음악 기초 사실) ③ BlogPostCTA 앵커 2종 ④ 발행 빈도 주 1~2주제로 축소.
> 본 § 0 하위 절과 § 7.8(BLOG_LOG 통합)은 새 정책 우선. 옛 학술 인용·DOI 관련 항목은 폐기 표시.

### 0.0 토픽 화이트리스트 (2026-06-13 신규 — 환각 차단)

**허용**: 위키 수준으로 확정된 음악 기초 사실·규칙 — 음표·쉼표·박자·박자표·음정·음계·조표·셈여림·기호의 정의와 규칙. 보편적 초견 기법(분산 연습·청크·시각 추적 등 일반화된 학습 원리)도 인용 없이 서술 가능.

**금지** (환각 위험):
- 음악사 해석·시대 분류 논쟁·양식 정의의 주관적 평가.
- 작곡가·연주자 일화·생애·심리 추정·특정 연도·특정 인물을 핵심으로 삼는 토픽.
- 특정 논문·연구 결과·통계 수치 제시.

**판단 기준**: 위키피디아 음악 기초 페이지에서 동일 내용을 인용 없이 확정적으로 서술하는 수준인지. 그 이상 정보 밀도가 필요하면 작성 건너뜀.

§ 3의 100개 주제 목록(blog_topics_100)에 화이트리스트 미부합 토픽이 포함되어 있어도 본 § 0.0 기준 미통과면 작성하지 않는다. 토픽 폐기 또는 무한 보류로 처리.

### 0.1 작성 페이스

**2026-06-13 갱신**: 검수 0 전제 정책 재설계 — 발행 빈도 대폭 축소.

- **주 1~2주제(ko+en 짝)**. 이전 페이스("하루 1주제·평일 2편·주말 3편")는 폐기.
- 토픽은 위키 수준 음악 기초 사실로 한정(§0 화이트리스트 참조) — 환각 위험 차단 우선, 분량 누적은 부수 목표.
- 이전 페이스 누적치(2026-05-30 기준 ko 69 + en 69)는 보존, 신규 작성만 새 빈도 적용.
...
### 0.7 학술 자료 활용 — 폐기 (2026-06-13)

> **폐기 사유**: 검수 0 전제에서 학술 인용은 출처 환각·DOI 오류·인용 정보 부정확 위험이 누적되고, 잘못된 인용 1건의 사이트 신뢰도·AdSense 노출 손해가 정확한 인용 N건의 효용을 초과한다.

**새 정책 (학술 인용 전면 금지)**:
- 본문 "(저자, 연도)" 인용·각주·`## 참고 문헌`·`## References` 섹션 모두 작성 금지.
- DOI 표기·저널명 인용·연구자 이름 인용 금지.
- 근거는 § 0.0 화이트리스트 범위의 음악 기초 사실·규칙 그 자체로 서술 — "박자표는 한 마디의 박자 수와 단위를 표시한다" 식의 정의·규칙 서술은 인용 없이 단정 가능.
...
또는 글 하단 `## 이미지 출처` (영문 글은 `## Image Sources`) 섹션에 통합:
...
> § 0.7 학술 인용 폐기로 이전의 `## 참고 자료` 통합 헤더는 사용하지 않는다. BlogPostCTA splitter 앵커가 `## 이미지 출처`/`## Image Sources` 2종으로 고정된 이상 다른 헤더명은 splitter 무동작.
...
한국어 + 영어 두 언어 동시 작성. § 0.1 새 빈도(주 1~2주제) 기준 — ko+en 짝으로 주 2~4편. (옛 "평일 2편 / 주말 3편" 페이스는 2026-06-13 폐기.)
...
#### 학술 인용 다양화 — 폐기 (2026-06-13)

> 학술 인용 전면 금지(§ 0.7) 정책으로 본 절 전체 무효. 저자·저널 풀, DOI 필수 규칙, 3편 연속 동일 저자 금지 등 모든 항목 적용 안 함.

#### history 추적

매 글 작성 시 직전 3편의 **이미지** history 확인(학술 인용 추적은 § 0.7 폐기로 제외):
...
또는 session-log §의 "완료 내역" 섹션에서 기록된 출처를 확인.

#### 블로그 sprint 통합 메시지 추가 줄

다음 sprint부터 매 블로그 작성 명령 통합 메시지에 다음 줄 추가:

> 직전 3편의 이미지 출처를 확인하고, 이번 글에서는 다른 출처를 사용. 이미지 다양화 정책(§0.11)을 따름. (학술 인용 다양화는 § 0.7 폐기로 적용 안 함.)
...
4. **Insight** — 통찰. 근거 = § 0.0 화이트리스트 범위의 음악 사실·규칙 그 자체 (학술 인용·DOI 작성 금지, § 0.7 참조).
...
학술 인용 전면 금지(§ 0.7). 이전 항목(DOI 필수·Peer-reviewed 저널·저자 풀 등) 모두 적용 안 함.
...
작업 사양: docs/marketing/blog_topics_100.md §0(특히 § 0.0 토픽 화이트리스트·§ 0.1 새 빈도·§ 0.7 학술 인용 금지) + §1.1(과거 기록 참고) 그대로 따름.
오늘 작성 분량: § 0.1 새 빈도(주 1~2주제, ko+en 짝). 옛 N일차 표는 과거 페이스 회고용으로만 참고.
...
- **토픽 화이트리스트 준수** (§0.0): 위키 수준 음악 기초 사실만. 음악사 해석·작곡가 일화·특정 인물 핵심·특정 논문 결과는 환각 위험으로 작성 금지. blog_topics_100 토픽 목록에 있어도 미부합이면 작성 건너뜀.
- **학술·논문·연구 인용 전면 금지** (§0.7): 본문 "(저자, 연도)" 인용·각주·DOI·저자·저널 언급·`## 참고 문헌`·`## References` 섹션 모두 작성 금지. 근거는 음악 기초 사실·규칙 그 자체로 서술.
- **이미지 활용** (§0.8 참조): 허용 출처(Wikimedia·British Library·LoC·Met·BnF·NYPL·Smithsonian·IMSLP·Mutopia·PLOS·PMC)만. Pexels·Unsplash·Pixabay·Freepik·AI 생성 금지. 모든 URL `curl -I` HTTP 200 확인. 글당 2장 이상.
- **본문 끝 H2 마커** = `## 이미지 출처`(ko) / `## Image Sources`(en) 2종 중 정확 일치. BlogPostCTA splitter 앵커. 누락 시 CTA 회귀.
...
- **다국어 작성** (§0.10 참조): 한국어 + 영어 두 언어 동시 작성. § 0.1 새 빈도(주 1~2주제, ko+en 짝) 기준. 같은 글은 한국어·영어 같은 slug 사용 (frontmatter `slug` 동일).
...
- `docs/active/PENDING_BACKLOG.md` §10.2 블로그 글 작성과 연동
- `docs/active/PENDING_BACKLOG.md` §3.1 AdSense 심사 전략과 연동
...
AdSense 심사 통과 필수 요건. `docs/active/PENDING_BACKLOG.md` §10에 별도 항목으로 등록.
...
### 7.8 BLOG_LOG.md 운영 (2026-06-13 갱신)

> 블로그 작성 로그(글 수 변동·이미지 정책 결정)는 `docs/active/BLOG_LOG.md`를 단일 진실로 사용. 본 § 7.8은 정책 자체(이미지·출처 섹션 구조)를 명시하고 BLOG_LOG는 이력 누적만 담당.

#### 7.8-A 본문 이미지 정책

- **모든 카테고리 공통**: 글당 이미지 2장 이상(§ 0.8 강화 정책 유지). 본문 내 자연 배치, Hook 섹션에는 이미지 두지 않음(Scene/Insight에).
- **본문 끝 H2 마커는 2종 한정**: `## 이미지 출처`(KO) / `## Image Sources`(EN). 이 H2가 BlogPostCTA의 splitter 앵커이므로 누락 금지.
...
#### 7.8-B 학술 인용 정책 — 폐기 (2026-06-13)

> 검수 0 전제(§ 0 헤더)로 학술 인용 전면 금지(§ 0.7). 이전 항목(DOI 검증·peer-reviewed 우선·인용 1개 이상 등) 모두 적용 안 함.

#### 7.8-C 출처 섹션 구조

학술 인용 금지로 본문 끝은 **이미지 출처 H2 1종만** 둔다.
...
영어 글은 `## Image Sources`. `## 참고 문헌` / `## References` 섹션 작성 금지.
...
| 2026-05-30 | 69 | 69 | 138 | §1-11 초견과 절대음감 (ko+en) 추가 |
...
| absolute-pitch-and-sight-reading | Wikimedia Mozart Prague Symphony autograph + Wikimedia Mozart Piano Concerto K.537 autograph | Levitin & Rogers (2005) DOI 10.1016/j.tics.2004.11.007 + Takeuchi & Hulse (1993) DOI 10.1037/0033-2909.113.2.345 |
