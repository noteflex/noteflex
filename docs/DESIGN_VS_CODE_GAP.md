...
|---|---|---|---|
| 정답률 | 80~85% 이상 | 80% (0.80) | ⚠️ 80% 또는 85%? |
| 반응속도 | 타이머 시간의 30~40% 이내 | (코드에 반응속도 통과 조건 **없음**) | ❌ |
| 수행 횟수 | 7~10번 이상 | 5회 이상 (`play_count >= 5`) | ❌ |
| 최대 연속 정답 | (설계엔 없음) | 5회 이상 (`best_streak >= 5`) | 💡 추가 구현 |

**불일치 발견 — 단계 Clear 기준**:

설계: 정답률 80~85% + 반응속도(타이머 30~40%) + 수행 7~10회
코드: 정답률 80% + 최대연속정답 5 + 수행 5회

→ **결정 필요**: 어느 기준으로 통일할지. 특히 **반응속도 통과 조건이 코드에 없음**.
...
| 같은 조표 연속 학습 (오답 多 시) | "혹은 해당 STAGE에서 틀린 음이 2개~3개 이상일 경우 N+2 로직을 그대로 적용하여 다음 STAGE에서도 동일한 조표의 문제로 적용하는 방안" | 🔴 미구현 — 결정 보류 | 결정 보류 |
...
| 항목 | 설계 (§3.3.아) | 구현 | 평가 |
|---|---|---|---|
| Lv5~7 골고루 비율 | "높은음자리표, 낮은음자리표, 조표가 섞여 있어 해당 경우의 수를 모두 반영" | `pickBalancedCount()` 로직 있으나 사용자 검증으로 **조표 비율 불충분 확인** | 🐛 |
| 6:4 / 4:6 / 7:3 / 3:7 비율 | (사용자 검증 시 명시) | 현재 비율 미확인 | 🐛 |
...
**현재 코드**: `🔁 재출제` 배지 표시 중 (`NoteGame.tsx`)
...
1. **단계 Clear 기준 통일** — 정답률 80% vs 80~85%, 반응속도 조건 추가, 수행 횟수 5 vs 7~10, 최대 연속 정답 조건
2. **미가입자 권한** — Lv 1만 (코드) vs Lv 1~2 (설계) 결정
3. **연간 구독료** — $24.99 확정 → Paddle Production 등록
4. **재도전 배지 삭제** — 설계에 명시된 우려 사항
5. **Sublevel 3 stage 수** — 3 stage (설계) vs 4 stage (코드) 통일
6. **첫 음표 버퍼링** — 카운트다운 후 안정화 지연 추가
...
5. ❌ 미가입자 권한 결정
6. ❌ Sublevel 3 stage 수 결정
7. ⚠️ 카운트다운 후 안정화 지연 추가
...
3. 🔴 가입자 대시보드 부분 잠금
4. 🔴 결제 후킹 메시지
5. 🔴 연간 구독 Production 등록
6. 🔴 성능 최적화 (performance.now, useRef, requestAnimationFrame)
