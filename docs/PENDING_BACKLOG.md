...
### §X-1 완료 (2026-05-10) ✅
- **A1** 이메일 OTP 가입 인증 (6자리 코드 모달, verifyOtp, 60초 cooldown) ✅
- **A3** 이메일 중복 검증 + 로그인 redirect CTA ✅
- **A5** 비밀번호 강도 검증 (8자+대소문자·숫자·특수문자, 실시간 바) ✅
- **D2** 전 테이블 RLS 검증 + 정정 (is_admin() 함수, 9개 테이블) ✅
...
- 2026-05-10 (일) (Sonnet 4.6): **11일차 블로그 3편 완료** (commits 7d40212·2896e78·3e3a9bb, 722/722 PASS, tsc 0) — §3-38 약점 음표 집중 학습법 (Ericsson et al. 1993 DOI), §5-56 피아노 초견 가이드 (Karpinski 2000 + Lehmann 2002), §7-79 가중치 학습 알고리즘 (Cepeda et al. 2006 DOI). 6 .md 파일 (ko+en), 이미지 6개 curl HTTP 200 검증 완료. 누적 49편.
