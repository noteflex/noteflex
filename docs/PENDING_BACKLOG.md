...
- [출시후-우선] **AdSense 심사 재개 — publisher ID 코드 prod 노출 필요** — 현재 prod Vercel에 `VITE_ADS_ENABLED` 미설정 → publisher ID 코드가 라이브 페이지에 없음. Google은 head Auto ads 스니펫 또는 body 광고 유닛으로 publisher ID가 실제 페이지에 있어야 심사 진행. 5/24 제출 심사 정체 원인. **순서**: ① `VITE_ADS_ENABLED=true` + `VITE_ADSENSE_PUBLISHER_ID=ca-pub-4314740126698954` Vercel Production 환경변수 설정 → Redeploy → ② Google이 페이지 크롤 후 심사 재개 → ③ 승인 메일 수신 후 슬롯 ID 15개(`VITE_ADSENSE_SLOT_*` 12개 포함) 추가 설정. 광고 유닛 코드가 없는 상태에서 publisher ID만 head에 노출해도 심사 트리거 가능(Auto ads 방식).
