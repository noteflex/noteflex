...
- [출시후-모니터링] **AdSense 심사 대기 중** — `index.html` head에 `ca-pub-4314740126698954` 로더 스크립트가 이미 정적으로 있고 라이브(noteflex.app·noteflex.app/blog) curl 검증으로 실제 서빙 확인(2026-06-03). publisher ID 노출 조건은 충족. 심사 정체 원인 미확정 (5/24 제출 후 최대 4주 소요 정상 범위, 콘텐츠 정책·크롤러 접근 여부 확인 필요). 승인 메일 수신 후: ① Vercel Production에 `VITE_ADS_ENABLED=true` + `VITE_ADSENSE_PUBLISHER_ID` + `VITE_ADSENSE_SLOT_*` 12개 설정 → Redeploy.
