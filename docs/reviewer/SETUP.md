...
> **권한**: Free tier (Lv1~5 Sub1 + 7회/일 + 광고 적용됨 + GAME_ENABLED 우회).
...
**Vercel Dashboard → Settings → Environment Variables → Production** 영역에 완료:
...
환경변수 기록한 후 Vercel **Redeploy** 완료 (재배포 적용된야 환경변수 적용).
...
### A. 영향 X 영역
- [ ] 일반 사용자가 메인 → 가입 → 매직링크 정상 흐름 적용됨
- [ ] 일반 사용자가 로그인 → 매직링크 정상 흐름 적용됨
- [ ] AuthModal Step 1·2·3 (가입·매직링크·복구 화면) 정상 적용됨

### B. reviewer 흐름
- [ ] AuthModal 풋터 "Paddle Reviewer Access" 링크 적용됨 (작은 텍스트)
- [ ] 클릭 → reviewer 패널 (이메일 + 액세스 코드 입력 폼) 적용됨
- [ ] `forpaddle@noteflex.app` + 정확한 코드 → 즉시 로그인 적용됨
- [ ] 로그인 후 게임 영역 진입 가능 (GAME_ENABLED 우회 적용됨)
- [ ] 로그인 후 Lv1~5 Sub1만 접근 적용됨 (Free tier 한도)
- [ ] 로그인 후 광고 노출 적용됨 (일반 사용자 영역 정합)
- [ ] 로그인 후 Premium 업그레이드 버튼 → Paddle Sandbox checkout 적용됨

### C. 보안 시나리오
- [ ] `forpaddle@noteflex.app` + 잘못된 코드 → 401 "Invalid credentials" 적용됨
...
Access code: {REVIEWER_ACCESS_CODE 값 완료}
...
**방법 2**: API 라우트 파일 삭제
1. `rm api/reviewer-login.ts` (사용자 직접 완료)
...
- [x] 실패 시 구체 사유 노출 X (`"Invalid credentials"` 만 완료)
- [x] reviewer = Free tier 격리 (Premium 영역 침범 X)
- [x] reviewer가 다른 사용자 데이터 조회 X (RLS 그대로 적용됨)
...
### "Invalid credentials" 적용됨 (정확한 코드인데도)
- Vercel 환경변수 `REVIEWER_ACCESS_CODE` 적용되어 있는지 확인
- 환경변수 기록한 후 Redeploy 완료했는지 확인
- 코드 앞뒤 공백 없는지 확인

### "Server error" 적용됨
- `SUPABASE_SERVICE_ROLE_KEY` 적용되어 있는지 확인
- `SUPABASE_URL` 적용되어 있는지 확인
- Vercel Function 로그 확인 (Vercel Dashboard > Functions)

### 로그인 적용됐는데 게임 영역 진입 X
- `forpaddle@noteflex.app` 의 `profiles.role = 'reviewer'` 적용되어 있는지 확인:
  ```sql
  SELECT role FROM profiles WHERE id IN (
    SELECT id FROM auth.users WHERE email = 'forpaddle@noteflex.app'
  );
  ```
- ComingSoonGate가 admin·reviewer 우회 적용되어 있는지 코드 확인
