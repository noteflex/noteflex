...
const mockGetUser               = vi.fn().mockResolvedValue({ data: { user: { id: "uid-oauth" } } });
...
describe("마운트 초기 상태", () => {
  ...
  it("initialSignupStep=3 으로 마운트 시 Step 3 렌더링, 닉네임 필드 비어있음", async () => {
    ...
    expect((screen.getByPlaceholderText(/3~20자/) as HTMLInputElement).value).toBe("");
  });
});

// ─────────────────────────────────────────────────────────────────────────
// isOAuthUser + Google OAuth redirectTo
// ─────────────────────────────────────────────────────────────────────────

describe("isOAuthUser — Step 3 비번 필드", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetUser.mockResolvedValue({ data: { user: { id: "uid-oauth" } } });
    mockCompleteProfile.mockResolvedValue({ error: null });
  });
